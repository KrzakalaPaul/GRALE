#!/bin/bash
#SBATCH --job-name=pubchem32_bottleneck_post          # nom du job
#SBATCH --output=logs/pubchem32_bottleneck_post.out      # nom du fichier de sortie
#SBATCH --error=logs/pubchem32_bottleneck_post.err       # nom du fichier d'erreur (ici commun avec la sortie)
...
srun python -u train.py --run_name pubchem32_bottleneck_post --dataset_path $SCRATCH/PUBCHEM_32.h5 --config bottleneck/post