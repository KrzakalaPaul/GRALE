#!/bin/bash
#SBATCH --job-name=pubchem32          # nom du job
#SBATCH --output=logs/pubchem32%j.out      # nom du fichier de sortie
#SBATCH --error=logs/pubchem32%j.err       # nom du fichier d'erreur (ici commun avec la sortie)
...
#SBATCH --cpus-per-task=96           # nombre de CPU par tache pour gpu_p5 (1/8 des CPU du noeud 8-GPU A100)
...
srun python -u train.py --run_name pubchem32_base --dataset PUBCHEM_32 --config 32