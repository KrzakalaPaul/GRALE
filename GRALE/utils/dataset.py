...
def load_in_memory(h5_dataset, start_idx=0, end_idx=-1):
    data_in_memory = {}
...
class LazyDataset():
    ...
    def get_data_epoch_starting_from(self, pointer):
        ...
        if end <= self.n_data:
            ...
            # wrap around
            subset1 = load_in_memory(self.lazy_data, start_idx=start, end_idx=-1)
...
class InMemoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        node_labels = torch.nn.functional.one_hot(torch.tensor(node_labels, dtype=torch.long), num_classes=self.metadata['n_node_labels']).to(torch.float)
        edge_labels = torch.tensor(edge_labels, dtype=torch.long)
        adjacency_matrix = (edge_labels > 0).to(torch.float)
        edge_labels = torch.nn.functional.one_hot(edge_labels, num_classes=self.metadata['n_edge_labels']).to(torch.float)
...
class MyDataModule(pl.LightningDataModule):
    def __init__(self, path_h5: str, batch_size: int = 32, n_data_epoch: int = -1, n_data_valid: int = -1):
        '''
        If n_data_epoch = -1, use the entire dataset each epoch and load it fully in memory.
    ...
    def train_dataloader(self):
        if self.lazy:
            ...
            return DataLoader(dataset_epoch, batch_size=self.batch_size, shuffle=True, collate_fn=custom_collate_fn)
        else:
            return DataLoader(self.dataset_train, batch_size=self.batch_size, shuffle=True, collate_fn=custom_collate_fn)
    
    def val_dataloader(self):
        return DataLoader(self.dataset_valid, batch_size=self.batch_size, shuffle=False, collate_fn=custom_collate_fn)