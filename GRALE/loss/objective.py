...
class GraphAutoencoderObjective():
    ...
    def __call__(self, outputs, targets, permutation_matrices):
        ...
        loss_h_batch = loss_h.sum()
        loss_node_features_batch = loss_node_features.sum()
        loss_node_labels_batch = loss_node_labels.sum()
        loss_edge_features_batch = loss_edge_features.sum()
        loss_edge_labels_batch = loss_edge_labels.sum()
        loss_adjacency_batch = loss_adjacency.sum()
        loss_marginals_batch = loss_marginals.sum()
        loss_batch = loss.sum()
        
        log = {'loss (batch)': loss_batch.item(),
               'loss_h (batch)': loss_h_batch.item(),
               'loss_node_features (batch)': loss_node_features_batch.item(),
               'loss_node_labels (batch)': loss_node_labels_batch.item(),
               'loss_edge_features (batch)': loss_edge_features_batch.item(),
               'loss_edge_labels (batch)': loss_edge_labels_batch.item(),
               'loss_adjacency (batch)': loss_adjacency_batch.item(),
               'loss_marginals (batch)': loss_marginals_batch.item()
               }
...
class GraphAutoencoderMetric():
    ...
    def __call__(self, outputs, targets, permutation_matrices):
        ...
        acc_graph = torch.where(edit_graph<1e-5, torch.tensor(0.), torch.tensor(1.))
        
        loss_h_batch = loss_h.sum()
        loss_node_labels_batch = loss_node_labels.sum()
        loss_edge_labels_batch = loss_edge_labels.sum()
        loss_adjacency_batch = loss_adjacency.sum()
        edit_graph_batch = edit_graph.sum()
        acc_graph_batch = acc_graph.sum()
        
        log = {'acc_h (batch)': loss_h_batch.item(),
               'acc_node_labels (batch)': loss_node_labels_batch.item(),
               'acc_adjacency (batch)': loss_adjacency_batch.item(),
               'acc_edge_labels (batch)': loss_edge_labels_batch.item(),
               'acc_graph (batch)': acc_graph_batch.item(),
               'edit_graph (batch)': edit_graph_batch.item()
               }
        
        return log
    
    def edit_batch(self, outputs, targets, permutation_matrices):
        
        h_preds = outputs.h
        node_labels_preds = outputs.nodes.labels
        edge_labels_preds = outputs.edges.labels
        edge_labels_preds = 0.5*(edge_labels_preds + edge_labels_preds.transpose(-3, -2))
        A_preds = outputs.edges.adjacency
        A_preds = 0.5*(A_preds + A_preds.transpose(-2, -1))
